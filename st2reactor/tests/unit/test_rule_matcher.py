...
class RuleMatcherTestCase(CleanDbTestCase):
    ...
    def test_rule_enforcement_is_created_on_exception_1(self):
        ...
        expected_failure = (
            ...
            '"trigger.k1": exception in _render_criteria_pattern' % (str(trigger_instance.id))
        )
    ...
    def test_rule_enforcement_is_created_on_exception_3(self):
        ...
        expected_failure = (
            ...
            'yoyohoneysingh.st2.test.rule4: exception in equals' % (str(trigger_instance.id))
        )
        self.assertEqual(rule_enforcement_dbs[0].failure_reason, expected_failure)
        self.assertEqual(rule_enforcement_dbs[0].trigger_instance_id, str(trigger_instance.id))
        self.assertEqual(rule_enforcement_dbs[0].rule['id'], str(rule_4_db.id))
    ...
    def _setup_sample_trigger(self, name):
        ...
        created = TriggerDB(
            name=name, pack='dummy_pack_1', type=trigtype.get_reference().ref, parameters={}
        )
...
class BackstopRuleMatcherTestCase(DbTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.models.update(
            fixturesloader.save_fixtures_to_db(fixtures_pack=PACK, fixtures_dict=FIXTURES_RULES)
        )
