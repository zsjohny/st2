...
class TestActionChainRunner(ExecutionDbTestCase):
    ...
    @mock.patch.object(
        action_db_util, 'get_liveaction_by_id', mock.MagicMock(return_value=DummyActionExecution())
    )
    @mock.patch.object(action_db_util, 'get_action_by_ref', mock.MagicMock(return_value=ACTION_1))
    @mock.patch.object(
        action_service,
    ...
    def test_chain_runner_broken_on_success_path_static_task_name(self, request):
        ...
        expected_msg = (
            'Unable to find node with name "c5" referenced in "on-success" ' 'in task "c2"'
        )
        self.assertRaisesRegexp(
            runnerexceptions.ActionRunnerPreRunError, expected_msg, chain_runner.pre_run
        )
    ...
    def test_chain_runner_broken_on_failure_path_static_task_name(self, request):
        ...
        expected_msg = (
            'Unable to find node with name "c6" referenced in "on-failure" ' 'in task "c2"'
        )
        self.assertRaisesRegexp(
            runnerexceptions.ActionRunnerPreRunError, expected_msg, chain_runner.pre_run
        )
    ...
    @mock.patch.object(action_db_util, 'get_action_by_ref', mock.MagicMock(return_value=ACTION_1))
    @mock.patch.object(
        action_service, 'request', return_value=(DummyActionExecution(result={'o1': '1'}), None)
    )
    def test_chain_runner_dependent_param_temp(self, request):
        ...
        chain_runner.run({'s1': 1, 's2': 2, 's3': 3, 's4': 4})
        self.assertNotEqual(chain_runner.chain_holder.actionchain, None)
        expected_values = [{u'p1': u'1'}, {u'p1': u'1'}, {u'p2': u'1', u'p3': u'1', u'p1': u'1'}]
    ...
    def test_chain_task_passes_invalid_parameter_type_to_action(self, mock_request):
        ...
        self.assertRaisesRegexp(
            ValueError, expected_msg, chain_runner.run, action_parameters=action_parameters
        )
    ...
    def test_action_chain_runner_referenced_action_doesnt_exist(self, mock_request):
        ...
        expected_error = (
            'Failed to run task "c1". Action with reference "wolfpack.a2" ' 'doesn\'t exist.'
        )
        self.assertEqual(status, LIVEACTION_STATUS_FAILED)
    ...
    def test_exception_is_thrown_if_both_params_and_parameters_attributes_are_provided(self):
        ...
        expected_msg = (
            'Either "params" or "parameters" attribute needs to be provided, but ' 'not both'
        )
        self.assertRaisesRegexp(
            runnerexceptions.ActionRunnerPreRunError, expected_msg, chain_runner.pre_run
        )
