...
class AliasExecutionTestCase(FunctionalTest):
    ...
    def test_invalid_format_string_referenced_in_request(self, request):
        ...
        post_resp = self._do_post(
            alias_execution=self.alias1, command=command, format_str=format_str, expect_errors=True
        )
    ...
    def test_execution_secret_parameter(self, request):
        ...
        self.assertEquals(request.call_args[0][0].parameters, expected_parameters)
        post_resp = self._do_post(
            alias_execution=self.alias4, command=command, show_secrets=True, expect_errors=True
        )
        self.assertEqual(post_resp.status_int, 201)
        self.assertEqual(
    ...
    def test_match_and_execute_matches_one_multiple_match(self, mock_request):
        ...
        data = copy.deepcopy(base_data)
        data['command'] = (
            'JKROWLING-4 is a duplicate of JRRTOLKIEN-24 which ' 'is a duplicate of DRSEUSS-12'
        )
        resp = self.app.post_json('/v1/aliasexecution/match_and_execute', data)
        self.assertEqual(resp.status_int, 201)
    ...
    def _do_post(
        self, alias_execution, command, format_str=None, expect_errors=False, show_secrets=False
    ):
        if isinstance(alias_execution.formats[0], dict) and alias_execution.formats[0].get(
            'representation'
        ):
