...
ST2_WEBHOOK = {'trigger': 'git.pr-merged', 'payload': {'value_str': 'string!', 'value_int': 12345}}

WEBHOOK_DATA = {'value_str': 'test string 1', 'value_int': 987654}
...
class TestWebhooksController(FunctionalTest):
    ...
    @mock.patch.object(WebhooksController, '_is_valid_hook', mock.MagicMock(return_value=True))
    @mock.patch.object(HooksHolder, 'get_all', mock.MagicMock(return_value=[DUMMY_TRIGGER_DICT]))
    ...
    def test_json_request_body(self, dispatch_mock):
        ...
        self.assertEqual(post_resp.status_int, http_client.ACCEPTED)
        self.assertEqual(
            dispatch_mock.call_args[1]['payload']['headers']['Content-Type'], 'application/json'
        )
        ...
        post_resp = self.app.post('/v1/webhooks/git', data, headers=headers, expect_errors=True)
        self.assertEqual(post_resp.status_int, http_client.BAD_REQUEST)
    ...
    def test_form_encoded_request_body(self, dispatch_mock):
        ...
        headers = {'Content-Type': 'application/x-www-form-urlencoded', 'St2-Trace-Tag': 'tag1'}
    ...
    def test_st2_webhook_array_webhook_array_input_type_not_valid(self):
        ...
        self.assertEqual(post_resp.status_int, http_client.BAD_REQUEST)
        self.assertEqual(
            post_resp.json['faultstring'], 'Webhook body needs to be an object, got: array'
        )
    ...
    def __do_post(self, hook, webhook, expect_errors=False, headers=None):
        return self.app.post_json(
            '/v1/webhooks/' + hook, params=webhook, expect_errors=expect_errors, headers=headers
        )
