...
class PacksControllerTestCase(FunctionalTest, APIControllerWithIncludeAndExcludeFilterTestCase):
    ...
    @mock.patch.object(
        pack_service, 'get_pack_from_index', mock.MagicMock(return_value=PACK_INDEX['test'])
    )
    ...
    @mock.patch.object(pack_service, 'get_pack_from_index', mock.MagicMock(return_value=None))
    ...
    @mock.patch.object(
        pack_service, '_build_index_list', mock.MagicMock(return_value=['http://main.example.com'])
    )
    ...
    @mock.patch.object(
        pack_service,
        '_build_index_list',
        mock.MagicMock(return_value=['http://main.example.com', 'http://broken.example.com']),
    )
    @mock.patch.object(requests, 'get', mock_index_get)
    ...
    @mock.patch.object(
        pack_service,
        '_build_index_list',
        mock.MagicMock(return_value=['http://fallback.example.com', 'http://main.example.com']),
    )
    @mock.patch.object(requests, 'get', mock_index_get)
    ...
    @mock.patch.object(
        pack_service,
        '_build_index_list',
        mock.MagicMock(return_value=['http://main.example.com', 'http://override.example.com']),
    )
    @mock.patch.object(requests, 'get', mock_index_get)
    ...
    def test_packs_register_endpoint_resource_register_order(self):
        ...
        expected_order = ['trigger', 'sensor', 'action', 'rule', 'alias', 'policy', 'config']
    ...
    def test_packs_register_endpoint(self, mock_get_packs):
        ...
        pack_names = ['dummy_pack_1', 'dummy_pack_2', 'dummy_pack_3', 'dummy_pack_10']
        mock_return_value = {}
        ...
        resp = self.app.post_json(
            '/v1/packs/register',
            {'packs': ['dummy_pack_2'], 'fail_on_failure': False, 'types': ['policies']},
        )
