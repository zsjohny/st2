...
CAST_OVERRIDES = {'array': (lambda cs_x: [v.strip() for v in cs_x.split(',')])}
...
class ActionAliasExecutionController(BaseRestControllerMixin):
    ...
    def _get_notify_field(self, payload):
        ...
        on_complete.data = {'user': payload.user, 'source_channel': payload.source_channel}
    ...
    def _schedule_execution(
        ...
        try:
            ...
            params = action_param_utils.cast_params(
                action_ref=action_alias_db.action_ref, params=params, cast_overrides=CAST_OVERRIDES
            )
            if not context:
                context = {
                    'action_alias_ref': reference.get_ref_from_model(action_alias_db),
            ...
            liveaction = LiveActionDB(
                action=action_alias_db.action_ref, context=context, parameters=params, notify=notify
            )
