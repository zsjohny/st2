...
class RunnerTypesController(ResourceController):
    ...
    def get_one(self, name_or_id, requester_user):
        return self._get_one_by_name_or_id(
            name_or_id, requester_user=requester_user, permission_type=PermissionType.RUNNER_VIEW
        )
    ...
    def put(self, runner_type_api, name_or_id, requester_user):
        ...
        rbac_utils.assert_user_has_resource_db_permission(
            user_db=requester_user, resource_db=runner_type_db, permission_type=permission_type
        )

        old_runner_type_db = runner_type_db
        LOG.debug(
            'PUT /runnertypes/ lookup with id=%s found object: %s', name_or_id, runner_type_db
        )
