...
class ParamsUtilsTest(DbTestCase):
    ...
    def test_unicode_value_casting(self):
        ...
        if six.PY3:
            expected = {'a1': (u'unicode1 ٩(̾●̮̮̃̾•̃̾)۶ unicode2')}
        else:
            expected = {
                'a1': (
    ...
    def test_get_finalized_params_with_casting_unicode_values(self):
        ...
        if six.PY3:
            expected_action_params = {'a1': (u'unicode1 ٩(̾●̮̮̃̾•̃̾)۶ unicode2')}
        else:
    ...
    def test_get_finalized_params_with_dict(self):
        ...
        self.assertEqual(
            r_action_params,
            {
                'a1': True,
                'a2': True,
                'a3': {'test': True, 'test1': 3, 'test2': ['1', True]},
                'a4': 3,
                'a5': ['1', True],
            },
        )
    ...
    def _get_liveaction_model(self, params, with_config_context=False):
        ...
        liveaction_db = LiveActionDB(
            status=status, start_timestamp=start_timestamp, action=action_ref, parameters=params
        )
    ...
    def test_get_value_from_datastore_through_render_live_params(self):
        ...
        live_params = param_utils.render_live_params(
            runner_parameters={}, action_parameters=param, params={}, action_context=context
        )
    ...
    def test_cyclic_dependency_friendly_error_message(self):
        runner_param_info = {
            'r1': {'default': 'some', 'cyclic': 'cyclic value', 'morecyclic': 'cyclic value'}
        }
        action_param_info = {'r2': {'default': '{{ r1 }}'}}
    ...
    def test_add_default_templates_to_live_params(self):
        ...
        schemas = [
            {'templateparam': {'default': '{{ 3 | int }}', 'type': 'integer', 'immutable': True}}
        ]
