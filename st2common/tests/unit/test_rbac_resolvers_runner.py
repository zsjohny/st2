...
__all__ = ['RunnerPermissionsResolverTestCase']
...
class RunnerPermissionsResolverTestCase(BasePermissionsResolverTestCase):
    def setUp(self):
        ...
        role_db = RoleDB(name='custom_role_runner_view_grant', permission_grants=permission_grants)
        ...
        permission_grants = [str(grant_db.id)]
        role_db = RoleDB(
            name='custom_role_runner_modify_grant', permission_grants=permission_grants
        )
        role_db = Role.add_or_update(role_db)
    ...
    def test_user_has_resource_db_permission(self):
        ...
        permission_types = [PermissionType.RUNNER_MODIFY, PermissionType.RUNNER_ALL]
        ...
        permission_types = [PermissionType.RUNNER_VIEW, PermissionType.RUNNER_ALL]
        self.assertUserDoesntHaveResourceDbPermissions(
            resolver=resolver,
            user_db=user_db,
            resource_db=resource_db,
