...
class PackActionAliasUnitTestUtils(BaseActionAliasTestCase):
    ...
    def test_assertCommandMatchesExactlyOneFormatString(self):
        ...
        expected_msg = (
            'Command "does not match foo" didn\'t match any of the provided format ' 'strings'
        )
        self.assertRaisesRegexp(
            AssertionError,
            expected_msg,
    ...
    @mock.patch.object(
        BaseActionAliasTestCase, '_get_base_pack_path', mock.Mock(return_value=PACK_PATH_1)
    )
    ...
    def _get_action_alias_db_by_name(self, name):
        ...
        values = {
            ...
            'pack': 'mock',
            'formats': ['show last {{count}} metrics for {{server}}'],
        }
