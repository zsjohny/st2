...
class ActionParamsUtilsTest(DbTestCase):
    ...
    def test_merge_action_runner_params_meta(self):
        required, optional, immutable = action_param_utils.get_params_view(
            action_db=self.action_dbs['action-1'], runner_db=self.runnertype_dbs['test-runner-1']
        )
        merged = {}
        ...
        self.assertTrue(
            'actionstr' not in optional and 'actionstr' not in immutable and 'actionstr' in merged
        )
        ...
        for opt in optional:
            self.assertTrue(
                opt not in required and opt not in immutable and opt in merged,
