...
__all__ = ['ExecutionPermissionsResolverTestCase']
...
class ExecutionPermissionsResolverTestCase(BasePermissionsResolverTestCase):
    def setUp(self):
        ...
        role_db = RoleDB(
            name='custom_role_unrelated_pack_action_grant', permission_grants=permission_grants
        )
        ...
        permission_grants = [str(grant_db.id)]
        role_db = RoleDB(
            name='custom_role_pack_action_view_grant', permission_grants=permission_grants
        )
        role_db = Role.add_or_update(role_db)
        ...
        grant_db = PermissionGrant.add_or_update(grant_db)
        permission_grants = [str(grant_db.id)]
        role_db = RoleDB(name='custom_role_action_view_grant', permission_grants=permission_grants)
        role_db = Role.add_or_update(role_db)
        ...
        permission_grants = [str(grant_db.id)]
        role_db = RoleDB(
            name='custom_role_pack_action_execute_grant', permission_grants=permission_grants
        )
        role_db = Role.add_or_update(role_db)
        ...
        grant_db = PermissionGrantDB(
            resource_uid=None, resource_type=None, permission_types=[PermissionType.EXECUTION_LIST]
        )
        grant_db = PermissionGrant.add_or_update(grant_db)
        permission_grants = [str(grant_db.id)]
        role_5_db = RoleDB(
            name='custom_role_execution_list_grant', permission_grants=permission_grants
        )
    ...
    def test_user_has_permission(self):
        ...
        self.assertUserHasPermission(
            resolver=resolver, user_db=user_db, permission_type=PermissionType.EXECUTION_LIST
        )
