...
__all__ = ['RuleEnforcementPermissionsResolverTestCase']
...
class RuleEnforcementPermissionsResolverTestCase(BasePermissionsResolverTestCase):
    def setUp(self):
        ...
        rule_enforcement_1_db = RuleEnforcementDB(
            ...
            rule={'ref': rule_1_db.ref, 'uid': rule_1_db.uid, 'id': str(rule_1_db.id)},
        )
        ...
        rule_enforcement_2_db = RuleEnforcementDB(
            trigger_instance_id=str(bson.ObjectId()),
            execution_id=str(bson.ObjectId()),
            rule={'ref': rule_2_db.ref, 'uid': rule_2_db.uid, 'id': str(rule_2_db.id)},
        )
        ...
        rule_enforcement_3_db = RuleEnforcementDB(
            trigger_instance_id=str(bson.ObjectId()),
            execution_id=str(bson.ObjectId()),
            rule={'ref': rule_3_db.ref, 'uid': rule_3_db.uid, 'id': str(rule_3_db.id)},
        )
        ...
        role_3_db = RoleDB(name='custom_role_rule_pack_grant', permission_grants=permission_grants)
        ...
        permission_grants = [str(grant_db.id)]
        role_4_db = RoleDB(
            name='custom_role_pack_rule_all_grant', permission_grants=permission_grants
        )
        role_4_db = Role.add_or_update(role_4_db)
        ...
        permission_grants = [str(grant_db.id)]
        role_5_db = RoleDB(
            name='custom_role_rule_modify_grant', permission_grants=permission_grants
        )
        ...
        grant_db = PermissionGrant.add_or_update(grant_db)
        permission_grants = [str(grant_db.id)]
        role_6_db = RoleDB(name='rule_pack_rule_create_grant', permission_grants=permission_grants)
        ...
        grant_db = PermissionGrant.add_or_update(grant_db)
        permission_grants = [str(grant_db.id)]
        role_7_db = RoleDB(name='rule_pack_rule_all_grant', permission_grants=permission_grants)
        ...
        grant_db = PermissionGrant.add_or_update(grant_db)
        permission_grants = [str(grant_db.id)]
        role_8_db = RoleDB(name='rule_rule_create_grant', permission_grants=permission_grants)
        ...
        grant_db = PermissionGrant.add_or_update(grant_db)
        permission_grants = [str(grant_db.id)]
        role_9_db = RoleDB(name='rule_rule_all_grant', permission_grants=permission_grants)
        role_9_db = Role.add_or_update(role_9_db)
        self.roles['rule_rule_all_grant'] = role_9_db

        # Custom role - "rule_list" grant
        grant_db = PermissionGrantDB(
            resource_uid=None, resource_type=None, permission_types=[PermissionType.RULE_LIST]
        )
        grant_db = PermissionGrant.add_or_update(grant_db)
        permission_grants = [str(grant_db.id)]
        role_10_db = RoleDB(name='custom_role_rule_list_grant', permission_grants=permission_grants)
