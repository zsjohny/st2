...
class TraceDBTest(CleanDbTestCase):
    ...
    def test_update_via_list_push_components(self):
        ...
        retrieved = Trace.push_components(
            ...
            rules=[TraceComponentDB(object_id=str(bson.ObjectId())) for _ in range(no_rules)],
            trigger_instances=[
                TraceComponentDB(object_id=str(bson.ObjectId()))
    ...
    def _create_save_trace(
        ...
        if action_executions is None:
            action_executions = []
        action_executions = [
            TraceComponentDB(object_id=action_execution) for action_execution in action_executions
        ]
        ...
        if trigger_instances is None:
            trigger_instances = []
        trigger_instances = [
            TraceComponentDB(object_id=trigger_instance) for trigger_instance in trigger_instances
        ]
