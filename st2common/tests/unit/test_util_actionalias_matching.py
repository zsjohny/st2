...
class ActionAliasTestCase(unittest2.TestCase):
    ...
    def test_list_format_strings_from_aliases(self, mock):
        ALIASES = [
            MemoryActionAliasDB(
                name="kyle_reese", ref="terminator.1", formats=["Come with me if you want to live"]
            ),
            MemoryActionAliasDB(
                name="terminator",
                ref="terminator.2",
    ...
    def test_list_format_strings_from_aliases_with_representation_only(self, mock):
        ...
        self.assertEqual(
            result[0]['representation'], "That's okay daddy. You can't hug a {{object}}."
        )
    ...
    def test_normalise_alias_format_string_error(self, mock):
        ...
        expected_msg = "alias_format '%s' is neither a dictionary or string type." % repr(
            alias_list
        )
    ...
    def test_matching(self, mock):
        ALIASES = [
            MemoryActionAliasDB(
                name="spengler", ref="ghostbusters.1", formats=["{{choice}} cross the {{target}}"]
            )
        ]
