...
class ActionDB(
    ...
    enabled = me.BooleanField(
        required=True, default=True, help_text='A flag indicating whether the action is enabled.'
    )
    entry_point = me.StringField(required=True, help_text='The entry point to the action.')
    pack = me.StringField(required=False, help_text='Name of the content pack.', unique_with='name')
    runner_type = me.DictField(
        required=True, default={}, help_text='The action runner to use for executing the action.'
    )
    parameters = stormbase.EscapedDynamicField(
        help_text='The specification for parameters for the action.'
    )
    output_schema = stormbase.EscapedDynamicField(help_text='The schema for output of the action.')
    notify = me.EmbeddedDocumentField(NotificationSchema)

    meta = {
        'indexes': [{'fields': ['name']}, {'fields': ['pack']}, {'fields': ['ref']}]
        + (
