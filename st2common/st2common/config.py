...
def register_opts(ignore_errors=False):
    ...
    system_opts = [
        ...
        cfg.StrOpt('base_path', default='/opt/stackstorm', help='Base path to all st2 artifacts.'),
        cfg.BoolOpt(
    ...
    content_opts = [
        cfg.StrOpt(
            'pack_group', default='st2packs', help='User group that can write to packs directory.'
        ),
        cfg.StrOpt(
    ...
    db_opts = [
        ...
        cfg.IntOpt(
            'connection_retry_max_delay_m', default=3, help='Connection retry total time (minutes).'
        ),
        cfg.IntOpt(
        ...
        cfg.IntOpt('connection_retry_backoff_mul', default=1, help='Backoff multiplier (seconds).'),
        cfg.BoolOpt('ssl', default=False, help='Create the connection to mongodb using SSL'),
        cfg.StrOpt(
            'ssl_keyfile',
            default=None,
    ...
    syslog_opts = [
        ...
        cfg.StrOpt('protocol', default='udp', help='Transport protocol to use (udp / tcp).'),
    ]
    ...
    log_opts = [
        ...
        cfg.BoolOpt('mask_secrets', default=True, help='True to mask secrets in the log files.'),
        cfg.ListOpt(
    ...
    api_opts = [
        ...
        cfg.BoolOpt('mask_secrets', default=True, help='True to mask secrets in the API responses'),
    ]
    ...
    auth_opts = [
        cfg.StrOpt(
            'api_url', default=None, help='Base URL to the API endpoint excluding the version'
        ),
        ...
        cfg.IntOpt(
            'service_token_ttl', default=(24 * 60 * 60), help='Service token ttl in seconds.'
        ),
    ]
    ...
    ssh_runner_opts = [
        ...
        cfg.StrOpt(
            'ssh_config_file_path', default='~/.ssh/config', help='Path to the ssh config file.'
        ),
    ]
    ...
    mistral_opts = [
        ...
        cfg.BoolOpt(
            'enable_polling', default=False, help='Enable results tracking and disable callbacks.'
        ),
        cfg.FloatOpt(
            'jitter_interval',
            default=0.1,
    ...
    query_opts = [
        cfg.IntOpt(
            'thread_pool_size', help='Number of threads to use to query external workflow systems.'
        ),
        cfg.FloatOpt(
            'query_interval',
    ...
    metrics_opts = [
        cfg.StrOpt('driver', default='noop', help='Driver type for metrics collection.'),
        cfg.StrOpt(
            'host',
            default='127.0.0.1',
